...
class SettingView extends StatelessWidget {
  final Color iconColor = const Color.fromARGB(255, 127, 92, 180);
  final Color textColor = const Color.fromARGB(255, 168, 126, 236);
  final Color selectColor = const Color.fromARGB(255, 103, 51, 145);
  final Color bgDarkColor = const Color.fromARGB(255, 54, 37, 79);

  final Color bgColor = const Color.fromARGB(150, 64, 46, 88);

  final Rx<String> selectMenu = 'Profile'.obs;

  final SettingController controller = Get.put(SettingController());

  final FocusNode _nickerFocusNode = FocusNode();
  final TextEditingController nickerController = TextEditingController();

  final FocusNode _accessTokenFocusNode = FocusNode();
  ...
  final FocusNode _searchUrlFocusNode = FocusNode();
  final TextEditingController _searchUrlController = TextEditingController();

  final FocusNode _bingSearchAccessTokenFocusNode = FocusNode();
  final TextEditingController bingSearchAccessTokenController =
  ...
  Widget build(BuildContext context) {
    nickerController.text = controller.setting.value.profileSetting.nicker;
    _nickerFocusNode.addListener(() {
      controller.setting.update((val) {
        val!.profileSetting.nicker = nickerController.text;
      });
    ...
    _accessTokenFocusNode.addListener(() {
      controller.setting.update((val) {
        val!.apiSetting.accessToken = accessTokenController.text;
      });
      Util.writeFile('settings.json', jsonEncode(controller.setting.value));
    });

    baseUrlController.text = controller.setting.value.apiSetting.baseUrl;
    _baseUrlFocusNode.addListener(() {
      controller.setting.update((val) {
        val!.apiSetting.baseUrl = baseUrlController.text;
      });
      Util.writeFile('settings.json', jsonEncode(controller.setting.value));
    });
    ...
    bingSearchAccessTokenController.text =
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              TextField(
                                ...
                                controller: bingSearchAccessTokenController,
